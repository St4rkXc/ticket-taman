...
@section('content')
    <aside
        class="fixed bottom-4 left-1/2 transform -translate-x-1/2 xl:translate-x-0 xl:bottom-0 w-fit xl:left-0 xl:w-[300px] h-fit xl:h-screen border-r-gray-200/200 border p-4 xl:p-8 bg-text xl:bg-transparent rounded-xl xl:rounded-none z-40">
        <p class="hidden xl:block text-[32px] font-bold text-text">Taman Kita</p>
        <div
            class="flex flex-row justify-normal xl:flex-col xl:justify-between h-full my-0 xl:my-12 pl-0 xl:pl-2 space-x-12 xl:space-x-0 ">
                    ...
                    <ul class="list-none mt-6">
                        @foreach ($bookingDetails as $booking)
                            <li class="flex justify-between items-center py-4 border-b border-b-white/20">
                                <div>
                                    <p class="text-white text-base font-bold leading-none">{{ $booking->nama }}</p>
                                    {{-- Nama Pengunjung --}}
                                    <div class="flex gap-2 mt-2">
                                        <p class="text-xxs font-semibold text-white/80 sm:hidden">
                                            {{ \Carbon\Carbon::parse($booking->created_at)->format('F d, Y') }}</p>
                                        {{-- Tanggal Booking --}}
                                        <p class="text-xxs font-light text-white hidden sm:block">Child : <span
                                                class="font-bold">{{ $booking->child_count }}</span></p>
                                        {{-- Tiket Anak --}}
                                        <p class="text-xxs font-light text-white hidden sm:block">Adult : <span
                                                class="font-bold">{{ $booking->adult_count }}</span></p>
                                        {{-- Tiket Dewasa --}}
                                    </div>
                                </div>
                                <p class="text-white text-base font-bold leading-none hidden sm:block">
                                    {{ \Carbon\Carbon::parse($booking->created_at)->format('F d, Y') }}</p>
                                {{-- Tanggal --}}
                            </li>
                        @endforeach
